...
public class TableVisual : MonoBehaviour 
{
    ...
    // a reference to a game object that marks positions where we should put new Creatures
...
public void AddCreatureAtIndex(CardAsset ca, int UniqueID ,int index)
    {
        ...
        GameObject Legend = GameObject.Instantiate(GlobalSettings.Instance.CreaturePrefab, slots.Children[index].transform.position, Quaternion.identity) as GameObject;

        // apply the look from CardAsset
        OneCreatureManager manager = Legend.GetComponent<OneCreatureManager>();
        manager.cardAsset = ca;
        manager.ReadCreatureFromAsset();

        // add tag according to owner
        foreach (Transform t in Legend.GetComponentsInChildren<Transform>())
            t.tag = owner.ToString()+"Creature";
        
        // parent a new creature gameObject to table slots
        Legend.transform.SetParent(slots.transform);

        // add a new creature to the list
        CreaturesOnTable.Insert(index, Legend);

        // let this creature know about its position
        WhereIsTheCardOrCreature w = Legend.GetComponent<WhereIsTheCardOrCreature>();
        w.Slot = index;
        w.VisualState = VisualStates.LowTable;

        // add our unique ID to this Legend
        IDHolder id = Legend.AddComponent<IDHolder>();
        id.UniqueID = UniqueID;

        // after a new Legend is added update placing of all the other Legends
